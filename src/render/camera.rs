...
impl Camera {
    ...
    fn viewport_half(&self) -> [f32; 2] {
        [
            self.viewport_size[0] * 0.5,
            self.viewport_size[1] * 0.5,
        ]
    }
    ...
    pub fn world_to_screen(&self, world: [f64; 2]) -> [f32; 2] {
        ...
        let rel = [
            (world[0] - self.center[0]) as f32,
            (world[1] - self.center[1]) as f32,
        ];
        let vp_half = self.viewport_half();
        [
            rel[0] * self.zoom + vp_half[0],
            rel[1] * self.zoom + vp_half[1],
        ]
    }
    ...
    pub fn screen_to_world(&self, screen: [f32; 2]) -> [f64; 2] {
        let vp_half = self.viewport_half();
        let rel = [
            (screen[0] - vp_half[0]) / self.zoom,
            (screen[1] - vp_half[1]) / self.zoom,
        ];
        [
            self.center[0] + rel[0] as f64,
            self.center[1] + rel[1] as f64,
        ]
    }
    ...
    pub fn zoom_at(&mut self, pivot_screen: [f32; 2], factor: f32) {
        ...
        self.center = [
            world_before[0] - offset[0] as f64,
            world_before[1] - offset[1] as f64,
        ];
    }

    /// 현재 뷰포트가 world 좌표에서 덮는 범위(AABB). 컬링 판정에 씀.
    pub fn world_view_bounds(&self) -> ([f64; 2], [f64; 2]) {
        let a = self.screen_to_world([
            0.0, 0.0,
        ]);
