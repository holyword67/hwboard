...
// 5단계 펜 두께
pub const THICKNESS_LEVELS: [f32; 5] = [
...
pub fn layout(
    ...
    for (i, &t) in THICKNESS_LEVELS.iter().enumerate() {
        if (t - current_thickness).abs() < 0.1 {
            selected_idx = i;
            break;
        }
    }
    buttons.push(UiButton {
        rect: Rect {
            x: start_x,
            y: thickness_y + (BUTTON_SIZE - bar_h) * 0.5,
            w: bar_w,
            h: bar_h,
        },
        kind: ButtonKind::ThicknessBar {
...
pub fn hit_test(
    ...
    for b in layout(viewport, current_tool, current_color, current_thickness) {
        if b.rect.contains(pos) {
            ...
            if let ButtonKind::ThicknessBar {
                ..
            } = b.kind
            {
