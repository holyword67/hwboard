...
impl App {
    ...
    pub fn render(&mut self) {
...

        {
            ...
            self.registry
                .sync(&self.core, &self.image_pipeline, &mut self.scene);
            ...
            for (id, item) in self.scene.iter_ordered_with_id() {
                ...
                match item {
                    CanvasItem::Stroke(s) => {
                        let Some(res) = self.registry.get_stroke(id) else {
                            continue;
                        };
                    ...
                    CanvasItem::Image(_) => {
                        let Some(res) = self.registry.get_image(id) else {
                            continue;
                        };
                        if !self.registry.is_visible(id, res.origin, view_min, view_max) {
                            continue;
            ...
            if let (Some(stroke), Some(mesh_cache)) =
                (&self.drawing_stroke, &self.drawing_mesh_cache)
            {
            ...
            if let Some(shape) = &self.drawing_shape_preview {
                ...
                if !mesh.indices.is_empty() {
                    ...
                    let immediate = DrawImmediate {
                        offset,
                        _pad: [0.0; 2],
                        color: self.boosted(shape.color),
                    };
                    pass.set_immediates(0, bytemuck::bytes_of(&immediate));
