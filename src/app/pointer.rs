...
impl App {
    ...
    fn handle_mouse_pointer(&mut self, ev: PointerEvent) {
        ...
        match ev {
            ...
            PointerEvent::Move(s) => {
                if self.panning {
                    let delta = [
                        s.pos[0] - self.last_pan_pos[0],
                        s.pos[1] - self.last_pan_pos[1],
                    ];
                ...
                if let Some(snap) = &self.snap_state {
                    if let Some(shape) = &mut self.drawing_shape_preview {
                        match snap.kind {
                            ...
                            _ => {
                                let d0 = [
                                    snap.initial_pen[0] - snap.center[0],
                                    snap.initial_pen[1] - snap.center[1],
                                ];
                                let dist0 = (d0[0] * d0[0] + d0[1] * d0[1]).sqrt().max(1.0);
                                let d1 = [
                                    world[0] - snap.center[0],
                                    world[1] - snap.center[1],
                                ];
                                ...
                                shape.half_extent = [
                                    snap.initial_half_extent[0] * scale,
                                    snap.initial_half_extent[1] * scale,
                                ];
    ...
    fn feed_smoother(&mut self, raw: PenPoint) {
        match (self.smoother_prev2, self.smoother_prev1.clone()) {
            (Some(a), Some(b)) => {
                let smoothed_pos = [
                    SMOOTHING_WEIGHTS[0] * a[0]
                        + SMOOTHING_WEIGHTS[1] * b.pos[0]
                        + SMOOTHING_WEIGHTS[2] * raw.pos[0],
                    SMOOTHING_WEIGHTS[0] * a[1]
                        + SMOOTHING_WEIGHTS[1] * b.pos[1]
                        + SMOOTHING_WEIGHTS[2] * raw.pos[1],
                ];
                self.push_finalized_point(PenPoint {
                    pos: smoothed_pos,
                    pressure: b.pressure,
                });
